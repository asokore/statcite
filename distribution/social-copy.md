...
Data: World Bank WDI, IMF WEO/Fiscal Monitor (current vintage, via the IMF's own DataMapper API), ECB reference rates. 42 active curated indicators, 200+ economies, inflation adjustment, historical FX (including ~90 currencies the ECB set doesn't cover, pegged Caribbean currencies convert exactly).
...
2/ The part I care about as an economist: verify_stat doesn't just say "wrong". It diagnoses HOW it's wrong. Claim matches last year's value? percent-vs-decimal slip? millions vs billions? Then it returns the correct figure with the citation attached.
