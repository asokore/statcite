...
4. **The competitive gap is real and specific.** The macro-data *access* layer is crowded (Google Data Commons MCP, official World Bank MCP, Trading Economics, ~30 thin wrappers). But as of July 2026 **no product returns per-number citation payloads, and none has a "verify this stat" tool** — the citation/verification layer is open. Licensing supports it: World Bank (CC BY 4.0), IMF, OECD, Eurostat, ECB, BLS all permit attributed commercial redistribution; FRED needs care (operator key, mandated disclaimer, third-party series).
...
**Crowded (access layer):** Google Data Commons MCP (free, hosted, Google-backed — the biggest strategic threat), official World Bank Data360 MCP (Python, anti-hallucination framing, no citations), Trading Economics MCP (commercial, 300k series), Alpha Vantage/EODHD (partial macro), OpenBB (self-host), ~30 single-source wrappers (FRED×4, Eurostat×3, OECD, ECB, BLS…), mostly local-stdio, thin, and per one audit 52% of listed MCP servers are dead. Largest observed traction for an econ MCP: AusEcon at 12.5k Smithery uses — the niche is early.

**Open (this product):**
- Per-number **citation payloads** (source, dataset, series id, canonical URL, license, retrieval date): no incumbent does it.
- **verify_stat** (claimed value vs official series → verdict + diagnostics): no incumbent has it; existing "fact check" tools are web-search-evidence or bibliographic checkers, none checks numbers against statistical APIs.
- **Provenance-preserving transforms** (inflation adjustment, FX bridging with method disclosure): scattered, nowhere audit-grade.

**Threats:** (1) Data Commons adds provenance + verification (natural extension); (2) a funded agent-data platform (Valyu already returns search-style citations) ships a verify endpoint; (3) the platform layer (Anthropic Citations API, retrieval-grounded modes) shrinks the problem. Realistic window as an independent layer: 18–36 months — speed of distribution matters more than feature completeness.
