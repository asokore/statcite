...
test("licence ledger: every source carries a verdict, note, and verified-on date; refused sources are present", async () => {
  ...
  for (const refused of ["fred", "un_comtrade"]) {
  ...
  // CBB followed on 2026-08-14 once citations could name the publication a
  // figure came from instead of asserting a currency stamp it never had.
  assert.equal((byId.get("cbb") as any)?.license_verdict, "served");
