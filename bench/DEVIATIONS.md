...
## D-008 — 2026-08-05 — Run R2 — Pre-registered amendments for Full Run 2 (logged before the draw; no model has been called)

Logged under COVENANT §7 as analysis-plan/prompt/roster changes relative to the frozen 1.0.0-P0 methodology, all committed and pushed BEFORE the R2 bank is drawn and before any model call. Where the methodology itself commands the change from the first full wave, the entry records execution, not deviation.

1. **Seeding (§6, commanded):** R2's master seed derives from a pre-announced NIST Randomness Beacon 2.0 pulse — `questions/R2-seed-announcement.json` commits the pulse time and the derivation formula before the pulse exists; `tools/beacon_seed.mjs` fetches and verifies it. Deviation component: the announcement lead time is minutes, not days — the announcement commit's push timestamp versus the pulse timestamp is the auditable gap.
2. **Contamination protocol (§6, commanded; late core selection is the deviation):** the frozen core panel should have been selected at pilot; it is selected now, mechanically (`tools/select_core_panel.mjs`: seeded shuffle of R1 headline qids, first 10 per tier, 40 total, seed derived from the beacon master). Fresh majority = 60. The core-vs-fresh gap is published. R1/P0's identical-bank reuse remains logged as D-007 item 1.
3. **Null probes (§3.4, commanded):** 25 via the generator's new `--null-probes` parameter.
4. **Roster (§6, commanded; rule text new):** the mechanical roster rule and its 2026-08-05 vendor snapshots are in `models.json`. The rule's exclusions (specialist ids, `-lite`, floating aliases, dated duplicates, codename previews) and the prior-wave-carryover clause are part of this amendment. gpt-5.5-pro's API-surface support is capability-probed at call time; a model neither chat/completions nor Responses can serve becomes a logged access-gap deviation.
5. **As-deployed arm (§2.5, commanded; scope note):** run as sibling run R2D on the same frozen bank/snapshots. Materially distinct only for OpenAI models; the Claude and Gemini arms' uniform settings already coincide with vendor defaults in this harness, so their as-deployed numbers equal their uniform numbers by construction — disclosed, not double-run.
6. **Retrieval-delta arm (§0, commanded; scope new):** sibling run R2V on a seeded 30-question stratified subset (deriveSeed(master, "retrieval-subset"): shuffle R2 headline, first 30 with tier balance 8/8/7/7), one flagship per vendor chosen mechanically by highest R1 uniform WTR (claude-opus-5, gpt-5.5, gemini-3-flash-preview), vendor-native retrieval only (web_search / google_search / Claude Code web search), StatCite banned by name in the committed retrieval prompt (`templates/system_prompt_retrieval_v1.txt`). Reported as a delta versus the same models' uniform-arm scores on the identical subset.
7. **Batching (§2.3):** retained at 10/call with the pre-registered controls; the "single-question calls if budget allows" intent is again not exercised (~9 models x 137 questions x up to 3 arms is outside this run's budget).
8. **New standard analyses (additive):** year-discrimination dual metric (ADDENDA §3), reweighting sensitivity with pre-frozen weights (§1.3, `snapshots/R2/weights.json`), parse-policy sensitivity (`tools/parse_sensitivity.mjs`), scoring-time revision events (§3.3.6, `tools/revision_events.mjs`), per-agent tool-call compliance counts for the Claude arm (`runs/R2/compliance.json`, closing ADDENDA §5). None alters the pre-registered headline definition.
9. **Publication sequencing (§8):** R2 results are withheld from the public repo and statcite.com until the vendor courtesy-preview window (5 business days from notification) has elapsed. Pre-registration artifacts are pushed before model calls; raw results, scores and the report are committed locally and pushed only after the window. This inverts R1's D-004 failure.
