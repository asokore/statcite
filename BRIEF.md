...
|---|---|---|
| `server/` | Cloudflare Worker: stateless MCP (hand-rolled JSON-RPC, zero runtime deps) + REST mirror + 48-indicator registry, 42 active + 5 data adapters (World Bank, IMF DataMapper, DBnomics, Frankfurter, FRED) | **253/253 tests pass (v1.11.0); live smoke passes against real World Bank / DBnomics / Frankfurter APIs** |
...
**v1.4.2 answered a fourth external review** (website-focused, 2026-07-27). Its verified findings were doc drift (`modeled_estimate` missing from docs.html's enum, a stale "every response repeats the revision caveat" sentence, `llms-full.txt` showing the pre-1.4.1 two-field `as_of` shape and omitting `as_of` from the REST parameter list, `get_series` still implying a server FRED key could work) plus a real machine-facing gap: the six disabled FRED-reserved keys looked normal in `/v1/indicators` and `search_indicators` output. Now both carry `active` (+ `disabled_reason`), and a disabled key's search result says "do not call". Also shipped from it: honest rewording of the Tow Center statistic and the arXiv-preprint label, "SLA-backed platform" replaced with plain Apify-terms language, WAI-ARIA tab semantics + skip link + copy announcement, JSON-LD/Twitter metadata, an illustrative-response label + a mismatch/diagnostics example, and a live try-it verifier form on the homepage backed by `GET /v1/verify`. **One of its "confirmed errors" was itself wrong**: it claimed custom connectors are unavailable on Claude's Free plan, citing an Anthropic article that (fetched live) actually says Free includes one custom connector, the site's wording stands. External reviews keep paying for exactly one reason: every claim gets verified against primary sources before any change is made. Review 2 was 5-parts-in-6 wrong on licensing surfaces, review 4 wrong on its top-priority item.
...
World Bank coverage for small economies is thin and patchy; the maintainer has direct working familiarity with small-state fiscal data and knows the national sources, definitions, and their quirks. A "Caribbean economic data" layer, covering CARICOM members, national statistical offices, central bank releases and the correct treatment of pegged currencies, would be defensible in a way that "another World Bank wrapper" never is, because it encodes knowledge that isn't in any API. **If I were choosing one thing to build next, it might be this rather than more breadth.** Ask him.
...
- **`country_snapshot` requests a fixed set.** Not configurable: ten World Bank keys, plus government debt via the IMF path, plus five ECCB items for ECCU geographies. HOW MANY COME BACK VARIES, and no single number is correct: 11 for the United States and Germany, 10 for Jamaica and Guyana, 9 for Barbados, 5 for Anguilla and Montserrat (measured 2026-08-16). Do not put a fixed count in marketing copy. The site said "11 indicators" until this was measured, which overstated it for exactly the small economies this service exists to cover.
